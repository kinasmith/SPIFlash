...
void SPIFlash_Marzogh::_dmac_disable() {
  DMAC->DMAC_EN &= (~DMAC_EN_ENABLE);
}
// Enable DMA Controller.
void SPIFlash_Marzogh::_dmac_enable() {
  DMAC->DMAC_EN = DMAC_EN_ENABLE;
}
// Disable DMA Channel
void SPIFlash_Marzogh::_dmac_channel_disable(uint32_t ul_num) {
  DMAC->DMAC_CHDR = DMAC_CHDR_DIS0 << ul_num;
}
// Enable DMA Channel
void SPIFlash_Marzogh::_dmac_channel_enable(uint32_t ul_num) {
  DMAC->DMAC_CHER = DMAC_CHER_ENA0 << ul_num;
}
// Poll for transfer complete
bool SPIFlash_Marzogh::_dmac_channel_transfer_done(uint32_t ul_num) {
  return (DMAC->DMAC_CHSR & (DMAC_CHSR_ENA0 << ul_num)) ? false : true;
}
// start RX DMA
void SPIFlash_Marzogh::_dueSPIDmaRX(uint8_t* dst, uint16_t count) {
...
void SPIFlash_Marzogh::_dueSPIDmaRX(char* dst, uint16_t count) {
  _dmac_channel_disable(SPI_DMAC_RX_CH);
  DMAC->DMAC_CH_NUM[SPI_DMAC_RX_CH].DMAC_SADDR = (uint32_t)&SPI0->SPI_RDR;
  DMAC->DMAC_CH_NUM[SPI_DMAC_RX_CH].DMAC_DADDR = (uint32_t)dst;
  DMAC->DMAC_CH_NUM[SPI_DMAC_RX_CH].DMAC_DSCR =  0;
...
// start TX DMA
void SPIFlash_Marzogh::_dueSPIDmaTX(const uint8_t* src, uint16_t count) {
...
void SPIFlash_Marzogh::_dueSPIDmaCharTX(const char* src, uint16_t count) {
  static char ff = 0XFF;
  uint32_t src_incr = DMAC_CTRLB_SRC_INCR_INCREMENTING;
  if (!src) {
    src = &ff;
...
void SPIFlash_Marzogh::_dueSPIBegin() {
  PIO_Configure(
...
void SPIFlash_Marzogh::_dueSPIInit(uint8_t dueSckDivisor) {
...
uint8_t SPIFlash_Marzogh::_dueSPITransfer(uint8_t b) {
  Spi* pSpi = SPI0;
...
// SPI receive a byte
uint8_t SPIFlash_Marzogh::_dueSPIRecByte() {
  return _dueSPITransfer(0XFF);
}
// SPI receive multiple bytes
uint8_t SPIFlash_Marzogh::_dueSPIRecByte(uint8_t* buf, size_t len) {
  Spi* pSpi = SPI0;
  int rtn = 0;
#if USE_SAM3X_DMAC
  // clear overrun error
...
// SPI receive a char
int8_t SPIFlash_Marzogh::_dueSPIRecChar() {
  return _dueSPITransfer(0XFF);
}
// SPI receive multiple chars
int8_t SPIFlash_Marzogh::_dueSPIRecChar(char* buf, size_t len) {
  Spi* pSpi = SPI0;
  char rtn = 0;
#if USE_SAM3X_DMAC
  // clear overrun error
...
// SPI send a byte
void SPIFlash_Marzogh::_dueSPISendByte(uint8_t b) {
  _dueSPITransfer(b);
}

void SPIFlash_Marzogh::_dueSPISendByte(const uint8_t* buf, size_t len) {
  Spi* pSpi = SPI0;
#if USE_SAM3X_DMAC
...
// SPI send a char
void SPIFlash_Marzogh::_dueSPISendChar(char b) {
  _dueSPITransfer(b);
}
//SPI send multiple chars
void SPIFlash_Marzogh::_dueSPISendChar(const char* buf, size_t len) {
  Spi* pSpi = SPI0;
#if USE_SAM3X_DMAC
